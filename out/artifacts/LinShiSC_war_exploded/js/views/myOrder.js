var integralModule = angular.module("myOrderApp",[]);
			...
			function initBind($http, $scope) {
				$('#table_id_example').on('click', '.select', function () {
					...
					createXiaoxiao1Table(params);
					showModal($('#orderXiaoxiao1Modal'));
				});
			...
			var Xiaoxiao1Datatable=null;
			function createXiaoxiao1Table(oId){
				if(Xiaoxiao1Datatable!=null){
					Xiaoxiao1Datatable.destroy();
				}
				Xiaoxiao1Datatable=$('#table_Xiaoxiao1_example').DataTable({
						bLengthChange: false,
						searching: false,
					    ordering:  false,
					    language: {
